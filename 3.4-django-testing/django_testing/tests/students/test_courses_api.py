...
#     id = random.randint(1, 10)
#     name = 'Test_name_course'
#     response = client.patch(f'http://127.0.0.1:8000/api/v1/courses/?id={id}', data={'id': id, 'name': name})
...
def test_delete_courses(client, course_factory):
    courses = course_factory(_quantity=10)
    id = random.randint(1, 10)
    response = client.delete(f'http://127.0.0.1:8000/api/v1/courses/?id={id}')
    assert response.status_code == 204
    response = client.get(f'http://127.0.0.1:8000/api/v1/courses/?id={id}')
